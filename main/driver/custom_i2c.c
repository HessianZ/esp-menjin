...
#define i2c_master_wait_us      os_delay_us
#define i2c_master_wait(arg)    os_delay_us(arg * i2c_clock_multiple)

static uint16_t i2c_clock_multiple = 1;
...
void i2c_master_set_clock(uint32_t mul)
{
    i2c_clock_multiple = 1000000 / mul;

    ESP_LOGI(I2C_TAG, "i2c clock set to %d", i2c_clock_multiple);
}

uint32_t i2c_master_get_clock()
{
    return 1000000 / i2c_clock_multiple;
}